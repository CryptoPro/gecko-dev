;( function () {
    ...
    window.addEventListener("message", function (event) {
        ...
        if (ext_id !== "")
            window.postMessage("cadesplugin_extension_id_response:" + ext_id, "*");
    }, false);
    ...
    function get_extension_id() {
        try {
            if(!isSafari && browserInstance.runtime) {
                return browserInstance.runtime.id;
            }
        }
        catch (err) {
            cpcsp_console_log(LOG_LEVEL_ERROR, "Failed to get extension version: " + err);
...
//    }
    ...
    function connect(){
        if (!isSafari) {
            cpcsp_console_log(LOG_LEVEL_DEBUG, "content.js: !isSafari connect().");
            cpcsp_console_log(LOG_LEVEL_INFO, "content.js: !isSafari connect().");
            g_bg_port = browser.runtime.connectNative("ru.cryptopro.nmcades.content");
    ...
    window.addEventListener("message", function (event) {
        ...
        if (event.data === "cadesplugin_echo_request") 
        {
            ...
            if (isFireFox) {
                answer += "url:";
                answer += browser.runtime.getURL("nmcades_plugin_api.js");
            }
            if (isSafari) {
                answer += "url:";
